...
export function * addCartItem(action) {
    ...
    try {
        const { message, items } = addItem(cart.items, item, volIdIndex, quantity);
        if (message) {
...
export function * removeCartItem(action) {
    ...
    const cart = yield select(state => state.cart);
    try {
        const items = cart.items.splice(index, 1);
        yield put(responseSuccess({ items }));
    } catch(error) {
        yield put(responseFailure(error));
    }
...
export function * updateCartItem(action) {
    ...
    const cart = yield select(state => state.cart);
    try {
        const items = changeItemQuantity(cart.items, index, quantity);
        yield put(responseSuccess({ items }));
    } catch (error) {
        yield put(responseFailure(error));
    }
...
function addItem(items, itemRef, volIdIndex, quantity = 1) {
    try {
        var cartItem = {};
        var message;
