...
class Store extends Component {
    ...
    render() {
        ...
        if (isHomebrew) {
            pageContent = <AddHomebrewContainer items={this.props.store.itemsToShow} />;
        } else if (pageType === "sub" && categoryId) {
            let category = find(dataArr, { id: Number(categoryId) });
            pageContent = <SubCat category={category} />;
        } else if (pageType === "cards" && legendCategory && legendSubcategory) {
            let cardsNode = [];
            this.props.store.itemsToShow.map((item, i) => {
                cardsNode.push(this.getCard(item, i));
            });
