...
export const createReducer = (initialState, handlers, allowReset = true, finalizer = x => x) => (state = initialState, action) => {
...
export const createActionsStructure = (prefix, protoArray) => {
    const { types, actions } = protoArray.reduce(
        ({ types, actions }, { name, async }) => {
