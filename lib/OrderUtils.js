...
const checkForRemovedItems = (order, cart) => {
    return cart.items.filter(element => {
        return !order.items.find(item => item.MerchandiseID == element.MerchandiseID)
    });
}
...
export const shipAllTogether = (order, user) => {
    ...
    return order.items.map((item) => ({
            ...item,
            shipDate: new Date(shipDates[index]),
            deliveryDate: getDeliveryDate(order, user, new Date(item.shipDate)),
            Warehouse: order.items[index].Warehouse
    }))
};
...
export const validateOrder = (order, user) => {
    ...
    const { id, shipmethod, shipping, billing } = user;
