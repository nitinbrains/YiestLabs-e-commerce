        ...
        else if(message.get)
        {
            try
            {
                ...
                if(shipIndex >= 0) {
                    ...
                    message.shipid = customerRecord.getSublistValue({sublistId: 'addressbook', fieldId: 'id', line: shipIndex});
                    message.shipattn = shipSubrecord.getValue({fieldId: 'attention'});
                    message.shipaddressee = shipSubrecord.getValue({fieldId: 'addressee'});
                    message.shipaddress1 = shipSubrecord.getValue({fieldId: 'addr1'});
                    message.shipaddress2 = shipSubrecord.getValue({fieldId: 'addr2'});
                    message.shipaddress3 = shipSubrecord.getValue({fieldId: 'addr3'});
                    message.shipcity = shipSubrecord.getValue({fieldId: 'city'});
                    message.shipzip = shipSubrecord.getValue({fieldId: 'zip'});
                    message.shipcountryid = shipSubrecord.getValue({fieldId: 'country'});
                }
               

                if(billIndex >= 0) {
                    var billSubrecord = customerRecord.getSublistSubrecord({sublistId: 'addressbook', fieldId: 'addressbookaddress', line: billIndex});
                    message.billid = customerRecord.getSublistValue({sublistId: 'addressbook', fieldId: 'id', line: billIndex});
                    message.billattn = billSubrecord.getValue({fieldId: 'attention'});
                    message.billaddressee = billSubrecord.getValue({fieldId: 'addressee'});
                    message.billaddress1 = billSubrecord.getValue({fieldId: 'addr1'});
                    message.billaddress2 = billSubrecord.getValue({fieldId: 'addr2'});
                    message.billaddress3 = billSubrecord.getValue({fieldId: 'addr3'});
                    message.billcity = billSubrecord.getValue({fieldId: 'city'});
                    message.billzip = billSubrecord.getValue({fieldId: 'zip'});
                    message.billcountryid = billSubrecord.getValue({fieldId: 'country'});
                }
        ...
        else
        {
            try
            {
                ...
                for(var i = 0; i < message.subsidiary.length; i++)
                {
                    ...
                    if(!message.shipcountryid)
                    {
                        throw {message: 'Missing Country for Shipping Address', code: 0};
                    }
                    customerRecord.setValue({fieldId: 'currency', value: getCurrencyFromSub(message.subsidiary[i], 'DK' == message.shipcountryid)});
                    customerRecord.setValue({fieldId: 'shippingitem', value: getShipMethodFromSub(message.subsidiary[i], 'DK' == message.shipcountryid, message.category)});
                    ...
                    shipSubrecord.setValue({fieldId: 'country', value: message.shipcountryid});

                    if(message.shipzip)
                    {
                        shipSubrecord.setValue({fieldId: 'zip', value: message.shipzip});
                    }
                    ...
                    shipSubrecord.setValue({fieldId: 'city', value: message.shipcity});
                    
                    if(message.shipattn)
                    {
                       shipSubrecord.setValue({fieldId: 'attention', value: message.shipattn}); 
                    }
                    
                    if(!message.shipaddressee)
                    {
                        throw {message: 'Missing Addressee for Shipping Address', code: 0};
                    }
                    shipSubrecord.setValue({fieldId: 'addressee', value: message.shipaddressee});

                    if(!message.shipaddress1)
                    {
                        throw {message: 'Missing Shipping Address Line 1', code: 0};
                    }
                    shipSubrecord.setValue({fieldId: 'addr1', value: message.shipaddress1});
                    
                    if(message.shipaddress2)
                    {
                        shipSubrecord.setValue({fieldId: 'addr2', value: message.shipaddress2});
                    }

                    if(message.shipaddress3)
                    {
                        shipSubrecord.setValue({fieldId: 'addr3', value: message.shipaddress3});
                    }             
                    ...
                    if(!message.billcountryid)
                    {
                        throw {message: 'Missing Country for Billing Address', code: 0};
                    }
                    billSubrecord.setValue({fieldId: 'country', value: message.billcountryid});

                    if(!message.billcity)
                    {
                        throw {message: 'Missing City for Billing Address', code: 0};
                    }
                    billSubrecord.setValue({fieldId: 'city', value: message.billcity});

                    if(message.billzip)
                    {
                        billSubrecord.setValue({fieldId: 'zip', value: message.billzip});
                    }
                    
                    if(message.billattn)
                    {
                       billSubrecord.setValue({fieldId: 'attention', value: message.billattn}); 
                    }
                    
                    if(!message.billaddressee)
                    {
                        throw {message: 'Missing Addressee for Billing Address', code: 0};
                    }
                    billSubrecord.setValue({fieldId: 'addressee', value: message.billaddressee});

                    if(!message.billaddress1)
                    {
                        throw {message: 'Missing Billing Address Line 1', code: 0};
                    }
                    billSubrecord.setValue({fieldId: 'addr1', value: message.billaddress1});
                    
                    if(message.billaddress2)
                    {
                        billSubrecord.setValue({fieldId: 'addr2', value: message.billaddress2});
                    }

                    if(message.billaddress3)
                    {
                        billSubrecord.setValue({fieldId: 'addr3', value: message.billaddress3});
                    }                
                    ...
                    if(message.vat)
                    {
                        try
                        {
                            customerRecord.setValue({fieldId: 'vatregnumber', value: message.billcountryid + message.vat});
                        }
                        catch(err)
                        {
                        ...
                        customerRecord.setValue({fieldId: 'taxitem', value: taxCodeLookup(message.billcountryid)});
            ...
            else //normal change cust
            {
                ...
                if(message.shipChange)
                {
                    try
                    {
                        var found = false;
                        var numAddresses = customerRecord.getLineCount({sublistId: 'addressbook'});
                        log.debug({title: 'message.shipid', details: message.shipid});
                        for(var i = numAddresses - 1; i >= 0; i--)
                        {
                            var addressSubrecord = customerRecord.getSublistSubrecord({sublistId: 'addressbook', fieldId: 'addressbookaddress', line: i});
                            var addressId = customerRecord.getSublistValue({sublistId: 'addressbook', fieldId: 'id', line: i});
                            log.debug({title: 'ship addressId', details: addressId});
                            if(message.shipid == addressId)
                            {
                                found = true;
                                log.debug({title: 'found ship address', details: i});
                        ...
                        if(!found)
                        {
                            log.debug({title: 'new ship address', details: true});
                ...
                if(message.billChange)
                {
                    try
                    {
                        var found = false;
                        var numAddresses = customerRecord.getLineCount({sublistId: 'addressbook'});
                        log.debug({title: 'message.billid', details: message.billid});
                        for(var i = numAddresses - 1; i >= 0; i--)
                        {
                            var addressSubrecord = customerRecord.getSublistSubrecord({sublistId: 'addressbook', fieldId: 'addressbookaddress', line: i});
                            var addressId = customerRecord.getSublistValue({sublistId: 'addressbook', fieldId: 'id', line: i});
                            log.debug({title: 'bill addressId', details: addressId});

                            if(message.billid == addressId)
                            {
                                log.debug({title: 'found bill address', details: i});
                                found = true;
                        ...
                        if(!found)
                        {
                            log.debug({title: 'new bill address', details: true});
                            customerRecord.insertLine({sublistId: 'addressbook', line: numAddresses});
